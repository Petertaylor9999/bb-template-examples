...
Param(
    ...
    [string] $TemplateParametersFile = 'Parameters\azuredeploy.parameters.json',
    [string] $ArtifactStagingDirectory = '.',
    [string] $DSCSourceFolder = 'DSC',
    [switch] $ValidateOnly,
	[string] $SubscriptionId ="fef306f2-be6f-47db-badf-2635097ba622",
    [switch] $DeleteExistingResourceGroup = $True
)
...
} catch { Write-Host "AddUserAgent failed" }
...
if ($UploadArtifacts) {
    ...
    if ($ArtifactsLocationSasToken -eq $null) {		
		...
		Write-Host "creating temporary SAS token..."
...
if($DeleteExistingResourceGroup) { Remove-AzureRmResourceGroup -Name $ResourceGroupName -Force}
